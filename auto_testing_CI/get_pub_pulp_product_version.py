...
class GetPubAndPulpVersion():
	...
	def get_pub_build_name(self):
		self.pub_build = self.e2e_version_content_list[3].split('</td>')[1].split("</p>")[0].split('>')[2].split('pub')[1]
		self.build_name_list['pub'] = 'pub-hub' + self.pub_build

	def get_pub_rcm_pa_tool_build_name(self):
		self.pub_rcm_pa_tool_build = self.e2e_version_content_list[3].split('</td>')[1].split("</p>")[1].replace('<p>', "")
